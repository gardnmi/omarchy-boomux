...
test("uses a configurable sliding side pane with an active Workspace tree", () => {
  ...
  expect(workspaceRow).not.toContain('id: workspaceSessionsButton')
  expect(workspaceRow).not.toContain('root.toggleWorkspaceSessionRail(modelData)')
...
test("documents actual keyboard navigation and configuration mutation", () => {
  ...
  expect(readme).not.toContain("Session icon on a coordinated Workspace")
  expect(readme).not.toContain("Browse Sessions")
...
describe("Agent Sessions", () => {
  ...
  test("keeps the dormant Session rail inaccessible", () => {
    const panel = fs.readFileSync(new URL("../Panel.qml", import.meta.url), "utf8")
    const sidePane = fs.readFileSync(new URL("../SidePane.qml", import.meta.url), "utf8")
    const workspaceActions = panel.slice(panel.indexOf("function actionMenuActionsFor"),
      panel.indexOf("function moveActionMenu"))
    const manifest = require("../manifest.json")
    expect(panel).toContain('var tabs = ["agents", "nodes"]')
    expect(panel).not.toContain('id: workspaceSessionsButton')
    expect(panel).not.toContain('text: "Browse Sessions"')
    expect(workspaceActions).not.toContain('actions.push("sessions")')
    expect(manifest.description).not.toContain("Session")
    expect(manifest.barWidget.description).not.toContain("Session")
    expect(manifest.barWidget.aliases).not.toContain("session")
    expect(manifest.barWidget.aliases).not.toContain("sessions")
