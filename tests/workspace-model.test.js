...
test("re-focuses an already-open pane through an idempotent IPC action", () => {
  ...
  expect(panel).toContain('sequence: "Meta+B"')
  expect(panel).toContain("enabled: root.opened")
  ...
  expect(sidePane).toContain("property bool focusIndicatorVisible: false")
  expect(sidePane).toContain("interval: 1800")
  expect(sidePane).toContain("border.width: root.focusIndicatorVisible")
  expect(sidePane).toContain("visible: !focusRemapping && (open || revealProgress > 0)")
  expect(sidePane).toContain("id: focusReacquireTimer")
  expect(sidePane).toContain("interval: 50")
  expect(panel).toContain('panel.focusIndicatorVisible ? "BOOMUX · KEYBOARD" : "BOOMUX"')
  expect(panel).toContain("panel.indicateKeyboardFocus()")
  expect(sidePane).toContain("focusPrimeTimer.restart()")
  expect(readme).toContain('"omarchy-shell io.github.gardnmi.boomux toggle"')
